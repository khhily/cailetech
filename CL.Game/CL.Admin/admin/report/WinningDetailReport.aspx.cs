...
{
    public partial class WinningDetailReport : AdminPage
    {
        protected int tradeType = 5;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("winningdetail_list", CaileEnums.ActionEnum.View.ToString()); //检查权限
        ...
        private void RptBind(int _tradeType, string _userName, string _mobile, string _startTime, string _endTime, string _orderBy)
        {
            ...
            string pageUrl = Utils.CombUrlTxt("winningdetailreport.aspx", "tradeType={0}&userName={1}&mobile={2}&startTime={3}&endTime={4}&page={5}", _tradeType.ToString(), _userName, _mobile, _startTime, _endTime, "__id__");
            PageContent.InnerHtml = Utils.OutPageList(this.pageSize, this.page, this.totalCount, pageUrl, 8);
        }
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            Response.Redirect(Utils.CombUrlTxt("winningdetailreport.aspx", "tradeType={0}&userName={1}&mobile={2}&startTime={3}&endTime={4}", this.tradeType.ToString(), this.txtUserName.Text, this.txtMobile.Text, this.txtStartTime.Text, this.txtEndTime.Text));
        }
        private int GetPageSize(int _default_size)
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("winningdetail_list_page_size", "QPcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("winningdetail_list_page_size", "QPcmsPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("winningdetailreport.aspx", "tradeType={0}&userName={1}&mobile={2}&startTime={3}&endTime={4}", this.tradeType.ToString(), this.userName, this.mobile, this.startTime, this.endTime));
        }
