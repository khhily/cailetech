...
namespace CL.Admin.UI
{
    public class UpLoad
    {
        ...
        public string ExcelTOLst(HttpPostedFile uploadfile, bool isThumbnail, bool isWater)
        {
            try
            {
                string fileExt = Utils.GetFileExt(uploadfile.FileName); //文件扩展名，不含“.”
                ...
                //检查文件扩展名是否合法
                if (string.IsNullOrEmpty(fileExt) || !fileExt.Contains("xls"))
                {
                ...
                //保存文件
                uploadfile.SaveAs(fullUpLoadPath + newFileName);

                return ExcelDataSource(fullUpLoadPath + newFileName, "出票明细统计");
                //插入数据到文件表
                //处理完毕，返回JOSN格式的文件信息
            }
            catch
        ...
        public string ExcelDataSource(string filepath, string sheetname)
        {
            try
            {
                string strConn;
                ...
                dt = ds.Tables[0];
                for (int i = 0; i <= dt.Rows.Count; i++) //LastRowNum 是当前表的总行数-1（注意）
                {
                    DataRow dr = dt.Rows[i];
                    if (dr != null)
                    {
                        OutETicketsEntity model = new OutETicketsEntity();
                        switch (dr[0].ToString().Trim())
                        {
                            case "红快3": model.LotteryCode = 101; break;
                            case "赢快3": model.LotteryCode = 102; break;
                            case "华11选5": model.LotteryCode = 201; break;
                            case "老11选5": model.LotteryCode = 202; break;
                            case "老时时彩": model.LotteryCode = 301; break;
                            case "江西时时彩": model.LotteryCode = 302; break;
                            case "双色球": model.LotteryCode = 801; break;
                            case "超级大乐透": model.LotteryCode = 901; break;
                        }
                        model.MerchantCode = 1;
                        model.MerchantTicket = dr[1].ToString().Trim();
                        model.HuaYangTicket = dr[2].ToString().Trim();
                        model.Money = Convert.ToInt64(dr[3].ToString().Trim().Replace("元", ""));
                        model.Multiple = Convert.ToInt32(dr[4].ToString().Trim());
                        model.Bonus = Convert.ToInt64(dr[5].ToString().Trim().Replace("元", ""));
                        model.SendTicketDateTime = Convert.ToDateTime(dr[6].ToString().Trim());
                        model.OutTicketDateTime = Convert.ToDateTime(dr[7].ToString().Trim());
                        switch (dr[8].ToString().Trim())
                        {
                            case "未出票": model.OutTicketStauts = 1; break;
                            case "出票成功": model.OutTicketStauts = 2; break;
                            default:
                                model.OutTicketStauts = 3; break;
                        }
                        new OutETicketsBLL().InsertEntity(model);
                    }
                }
    ...
    public string ReadFromExcelFile(string filePath)
        {
            ...
            try
            {
                ...
                for (int i = 1; i <= sheet.LastRowNum; i++) //LastRowNum 是当前表的总行数-1（注意）
                {
                    ...
                    {
                        OutETicketsEntity model = new OutETicketsEntity();
                        switch (row.GetCell(0).ToString().Trim())
                        {
                            case "红快3": model.LotteryCode = 101; break;
                            case "赢快3": model.LotteryCode = 102; break;
                            case "华11选5": model.LotteryCode = 201; break;
                        ...
                        model.MerchantCode = 1;
                        model.MerchantTicket = row.GetCell(1).ToString().Trim();
                        model.HuaYangTicket = row.GetCell(2).ToString().Trim();
                        model.Money = Convert.ToInt64(row.GetCell(3).ToString().Trim().Replace("元", ""));
                        model.Multiple = Convert.ToInt32(row.GetCell(4).ToString().Trim());
                        model.Bonus = Convert.ToInt64(row.GetCell(5).ToString().Trim().Replace("元", ""));
                        model.SendTicketDateTime = Convert.ToDateTime(row.GetCell(6).ToString().Trim());
                        model.OutTicketDateTime = Convert.ToDateTime(row.GetCell(7).ToString().Trim());
                        switch (row.GetCell(8).ToString().Trim())
                        {
                            case "未出票": model.OutTicketStauts = 0; break;
                            case "出票成功": model.LotteryCode = 2; break;
                            default:
                                model.LotteryCode = 4; break;
                        }
                        new OutETicketsBLL().InsertEntity(model);
                    }
                }
        ...
        /// <param name="isWater">是否打水印</param>
        /// <returns>上传后文件信息</returns>
        public string fileSaveAsUpLoadify(HttpPostedFile uploadfile, string fileSign, bool isThumbnail, bool isWater)
        {
            try
            {
                string fileExt = Utils.GetFileExt(uploadfile.FileName); //文件扩展名，不含“.”
                int fileSize = uploadfile.ContentLength; //获得文件大小，以字节为单位
                string fileName = uploadfile.FileName.Substring(uploadfile.FileName.LastIndexOf(@"\") + 1); //取得原文件名
                string newFileName = Utils.GetRamCode() + "." + fileExt; //随机生成新的文件名
                ...
                try
                {
                    SalePointFileEntity model = new SalePointFileEntity();
                    model.CreateTime = DateTime.Now;
                    model.FileUrl = newFilePath;
                    model.FileName = fileName;
                    model.FileEXT = fileExt;
                    model.FileSign = Convert.ToInt64(fileSign);
                    new SalePointFileBLL().InsertEntity(model);
                }
                catch
                {
                    return "{\"status\": 0, \"msg\": \"上传过程中发生意外错误！\"}";
