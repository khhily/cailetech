...
{
    public class SplitTicketOperation : RecTicketBase
    {
        #region 配置变量
        /// <summary>
        /// 消息队列
        ...
        /// 实时投注队列路径
        /// </summary>
        private string QueueBettingticket = String.Empty;

        private readonly Log log = new Log("ElectronicTicketReceiver");
        /// <summary>
        /// 请求停止状态
        /// </summary>
        ...
        public SplitTicketOperation(RichTextBox _infoBox) : base(_infoBox)
        {
            try
            {
                Initilize();
        ...
        private void Initilize()
        {
            ...
            QueueBettingticket = ini.Read(sectionName, "QueueBetting");
        ...
        private void ActionStart(ElectronicTicket order)
        {
            ...
            else //成功发送消息到出票队列
            {
                if (IsIsuseSale(order.LotteryCode, order.IsuseName))
                {
                    ...
                    //判断拆分大小 以便通知队列
                    TicketSender = new BettingTicketSender(QueueBettingticket);
