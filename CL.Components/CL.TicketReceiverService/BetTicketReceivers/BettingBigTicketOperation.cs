...
{
    public class BettingBigTicketOperation : RecTicketBase
    {
        private MessageQueue m_Queue;
        private int m_timespan = 30;
        private readonly Log m_log = new Log("BettingBigTicketOperation");
        private string QueueName = string.Empty;
        private XmlDocument doc = new XmlDocument();
        private SchemesBLL blls = new SchemesBLL();
        private SchemeETicketsBLL bll = new SchemeETicketsBLL();

        public BettingBigTicketOperation(RichTextBox _infoBox) : base(_infoBox)
        {
            try
            {
                                ...
                                try
                                {
                                    ...
                                    foreach (udv_BettingTickets item in TricketList)
                                    {
                                        udv_ParaBettingTicker model = new udv_ParaBettingTicker()
                                        {
                                            ...
                                            Bet = item.Bet, //item.Number.Split('^').Length, 
                                            Amount = item.Amount
                                        };
                                        para.Add(model);
                                    }
                                        ...
                                        if (SchemeEntity != null)
                                        {
                                            if (SchemeEntity.BuyType == (byte)BuyTypeEnum.BuyChase)
                                            {
                                                ...
                                                if (ChaseDetailEntity != null)
                                                {
                                                    ...
                                                    new ChaseTaskDetailsBLL().ModifyEntity(ChaseDetailEntity);
