...
namespace Dapper
{
    ...
    public static partial class SimpleCRUD
    {
        ...
        /// <param name="commandTimeout"></param>
        /// <returns>Gets a paged list of entities with optional exact match where conditions</returns>
        public static IEnumerable<T> GetListPaged<T>(this IDbConnection connection, int pageNumber, int rowsPerPage, object whereConditions, string orderby, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            if (string.IsNullOrEmpty(_getPagedListSql))
                throw new Exception("GetListPage is not supported with the current SQL Dialect");
            ...
            if (whereprops.Any())
            {
                ...
                //BuildWhere(conditions, whereprops, (T)Activator.CreateInstance(typeof(T)), whereConditions);
                conditions.AppendFormat(" where {0} ", whereConditions);
            }
            ...
            query = query.Replace("{RowsPerPage}", rowsPerPage.ToString());
            query = query.Replace("{OrderBy}", orderby);
            query = query.Replace("{WhereClause}", conditions.ToString());
            query = query.Replace("{Offset}", ((pageNumber - 1) * rowsPerPage).ToString());
        ...
        /// <param name="commandTimeout"></param>
        /// <returns>Gets a paged list of entities with optional exact match where conditions</returns>
        public static IEnumerable<T> GetListPaged<T>(this IDbConnection connection, int pageNumber, int rowsPerPage, object whereConditions, string orderby, object param, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            if (string.IsNullOrEmpty(_getPagedListSql))
                throw new Exception("GetListPage is not supported with the current SQL Dialect");
            ...
            return connection.Query<T>(query, param, transaction, true, commandTimeout);
        }

        public static IEnumerable<T> GetList<T>(this IDbConnection connection, object whereConditions, string orderby, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var idProps = GetIdProperties(currenttype).ToList();
            if (!idProps.Any())
        ...
        public static T Get<T>(this IDbConnection connection, object whereConditions, string orderby, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var idProps = GetIdProperties(currenttype).ToList();
            if (!idProps.Any())
            ...
            return connection.Query<T>(sb.ToString(), whereConditions, transaction, true, commandTimeout).FirstOrDefault();
        }
        #endregion
