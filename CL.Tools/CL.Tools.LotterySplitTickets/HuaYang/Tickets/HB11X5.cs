...
namespace CL.Tools.LotterySplitTickets.HuaYang.Tickets
{
    ...
    /// 2017年5月9日
    /// </summary>
    public class HB11X5 : TickBuilder
    {
        /// <summary>
        /// 彩种编号
        /// </summary>
        private const int LotteryCode = 201;
        private const int SuppId = 124;  //接口编码
            ...
            foreach (var ticket in tickets)
            {
                switch (ticket.PlayTypeCode)
                {
                    ...
                    case (int)HB11X5PlayCode.Q2_ZX:
                        ...
                        #endregion
                        break;
                    case (int)HB11X5PlayCode.Q2_ZUX:
                        #region 前二组选
                    ...
                    case (int)HB11X5PlayCode.Q3_ZX:
                        ...
                        #endregion
                        break;
                    case (int)HB11X5PlayCode.Q3_ZUX:
                        #region 前三组选
                        //示例：标准选号：单式：01,02,03 复式：01,02,03,04 胆拖选号：01,02#03,04、01#02,03,04
                        //单式：010203  复式：01020304  胆拖：01*020304、0102*0304
                        if (ticket.Number.IndexOf(SEP_NUM_B) > -1)  //是否胆拖
        ...
        private ICollection<udv_Ticket> MergeTicket(ICollection<udv_Ticket> tickets)
        {
            ...
            var s_list = MergeTicket(list.FindAll(x => x.PlayTypeCode != 20108), PickWay_DS, SINGLE_NUMBER_PER_TICKET, SEP_NUM_F);
            return list.FindAll(x => x.PlayTypeCode == 20108).Concat(s_list).ToList<udv_Ticket>();
        }
    }
}
