...
namespace CL.Tools.LotterySplitTickets.HuaYang.Tickets
{
    ...
    /// 2017年5月9日 
    /// </summary>
    public class JLXK3 : TickBuilder
    {
        /// <summary>
        /// 彩种编号
        /// </summary>
        private const int LotteryCode = 101;
        private const int SuppId = 126;  //接口编码
        ...
        private ICollection<udv_Ticket> MergeTicket(ICollection<udv_Ticket> tickets)
        {
            ...
            var s_list = MergeTicket(list.FindAll(x => x.PlayTypeCode != 10101), PickWay_DS, SINGLE_NUMBER_PER_TICKET, SEP_NUM_F);
            return list.FindAll(x => x.PlayTypeCode == 10101).Concat(s_list).ToList<udv_Ticket>();
        }
    }
}
