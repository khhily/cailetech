...
namespace CL.Tools.Common
{
    ...
    public static class PagingHelper
    {
        ...
        /// 获取记录总数SQL语句
        /// </summary>
        /// <param name="_tableSql">SQL查询表名</param>
        /// <param name="_whereSql">SQL查询条件</param>
        /// <returns>记录总数SQL语句</returns>
        public static string CreateCountingSql(string _tableSql, string _whereSql)
        {
            return string.Format(" SELECT COUNT(1) AS RecordCount FROM {0} WHERE {1} ", _tableSql, _whereSql);
        }
    }
}
