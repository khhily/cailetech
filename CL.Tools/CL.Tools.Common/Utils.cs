...
namespace CL.Tools.Common
{
    public class Utils
    {
        ...
        {
            return CLKeys.ASSEMBLY_VERSION;
        }
        #endregion

        #region MD5加密
        public static string MD5(string pwd, bool IsUTF8 = false)
        {
        ...
        {
            double tmp;
            if (double.TryParse(obj.ToString(), out tmp))
                return tmp / 100;
            else
                return 0;

            //return ObjToDecimal(obj) / 100;
        }
        ...
        {
            if (expression != null)
                return StrToDecimal(expression.ToString(), 0);
        ...
        /// <param name="o"></param>
        /// <returns></returns>
        public static bool isLNumber(object o)
        {
            long tmpInt;
            if (o == null)
            {
        ...
        public static void DeleteUpFile(string _filepath)
        {
            ...
            if (_filepath.LastIndexOf("/") >= 0)
            {
                ...
                string fullTPATH = GetMapPath(thumbnailpath);
        ...
        public static void RemoveCookie(string name, string key)
        {
            ...
            if (cookie != null)
            {
                ...
                HttpContext.Current.Response.Cookies.Remove(cookie[key]);//Request
            }
        }
        #endregion
        ...
        /// <returns></returns>
        public static string HttpPost(string url, string param, IDictionary<object, string> headers = null, bool IsUFT8 = false,
        ...
        /// <param name="url">URL.</param>
        /// <returns></returns>
        public static string HttpGet(string url, int Timeout = 2000, string UserAgent = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.0.2840.87 Safari/537.36")
        {
            HttpWebRequest request = (HttpWebRequest)HttpWebRequest.Create(url);
            request.Method = "GET";
        ...
        /// <param name="url">URL.</param>
        /// <returns></returns>
        public static string HttpGet(string url, Encoding enc, string UserAgent = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.0.2840.87 Safari/537.36")
        {
            HttpWebRequest request = (HttpWebRequest)HttpWebRequest.Create(url);
            request.Method = "GET";
            //request.ContentType = "application/x-www-form-urlencoded";
        ...
        /// <returns></returns>
        public static string HttpGetProxy(string url, string host, int port, Encoding ecode, string UserAgent = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.0.2840.87 Safari/537.36")
        {
            HttpWebRequest request = (HttpWebRequest)HttpWebRequest.Create(url);
            ...
            try
            {
                ...
                if (response != null)
                {
                    StreamReader reader = new StreamReader(response.GetResponseStream(), ecode);
                    responseStr = reader.ReadToEnd();
                    reader.Close();
                }
            }
        ...
        /// <returns></returns>
        public static string HttpSubmit(string url, SortedDictionary<string, string> postData, string httpMethod = "POST")
        {
            string result = "";
